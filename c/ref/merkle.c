...
void merkle_sign(crypto_context *cctx, uint8_t *sig, unsigned char *root,
  ...
  info.wots_sig = sig;
  chain_lengths(cctx, steps, root);
  info.wots_steps = steps;

  set_type(cctx, &tree_addr[0], SPX_ADDR_TYPE_HASHTREE);
  set_type(cctx, &info.pk_addr[0], SPX_ADDR_TYPE_WOTSPK);
  copy_subtree_addr(cctx, &info.leaf_addr[0], wots_addr);
  copy_subtree_addr(cctx, &info.pk_addr[0], wots_addr);

  info.wots_sign_leaf = idx_leaf;

  treehashx1(cctx, root, auth_path, ctx, idx_leaf, 0, SPX_TREE_HEIGHT,
...
void merkle_gen_root(crypto_context *cctx, unsigned char *root,
  ...
  set_layer_addr(cctx, top_tree_addr, SPX_D - 1);
  set_layer_addr(cctx, wots_addr, SPX_D - 1);

  merkle_sign(
      cctx, auth_path, root, ctx, wots_addr, top_tree_addr,
