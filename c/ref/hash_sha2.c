...
void gen_sha2_message_random(void *p_cctx, unsigned char *R,
  ...
  ASSERT(SPX_N <= SPX_SHAX_BLOCK_BYTES);
...
#define SPX_DGST_BYTES (SPX_FORS_MSG_BYTES + SPX_TREE_BYTES + SPX_LEAF_BYTES)
  ...
  ASSERT((SPX_SHAX_BLOCK_BYTES & (SPX_SHAX_BLOCK_BYTES - 1)) == 0)

#define SPX_INBLOCKS                                    \
  (((SPX_N + SPX_PK_BYTES + SPX_SHAX_BLOCK_BYTES - 1) & \
  ...
  if (SPX_N + SPX_PK_BYTES + mlen < SPX_INBLOCKS * SPX_SHAX_BLOCK_BYTES) {
    ...
    shaX_inc_finalize(seed + 2 * SPX_N, state, inbuf,
